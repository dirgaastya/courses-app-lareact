...
const EditPeriod = (props) => {
    ...
    const period = props.data.filter((period) => period.id === id);
    const { data, setData, errors, put, wasSuccessful } = useForm({
        id: period[0].id || "",
        name: period[0].name || "",
        time_start: period[0].time_start || "",
        time_end: period[0].time_end || "",
        cost: period[0].cost || 0,
    });
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        put(route("admin-period-update", period[0].id));
    }
    return (
        <div>
            <div className="py-4 mb-5 flex items-center justify-between">
                <div className="flex flex-col capitalize text-3xl ">
                    <span className="font-bold">
                        Edit <span>{period[0].name}</span> period{" "}
                    </span>
                </div>
                <Link
                    to="/admin/period"
